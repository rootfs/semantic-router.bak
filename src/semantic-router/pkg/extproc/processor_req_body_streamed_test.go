...
func TestHandler_InitState_TransitionsOnChunkSize(t *testing.T) {
	...
	require.True(t, len(body) > 32, "body must exceed chunkSize")
...
func TestHandler_IncrementalPreprocess_SplitsSentences(t *testing.T) {
	...
	assert.True(t, len(h.preproc.sentences) >= 2, "should split at least 2 sentences, got %d", len(h.preproc.sentences))
	assert.Equal(t, len(h.preproc.sentences), len(h.preproc.tfVectors), "TF vectors should match sentence count")
}
...
func TestHandler_MultiChunk_AutoModel_StateTransitions(t *testing.T) {
	...
	require.True(t, len(chunks) > 2, "need multiple chunks")
