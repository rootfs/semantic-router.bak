...
// ext_authz Injected Headers
// These headers are set by the ext_authz service after successful user token validation.
// They carry per-user provider API keys for external model routing.
const (
	// UserOpenAIKey carries the user's OpenAI API key, injected by ext_authz.
	// Used by the ext_proc when routing requests to OpenAI models.
	UserOpenAIKey = "x-user-openai-key"

	// UserAnthropicKey carries the user's Anthropic API key, injected by ext_authz.
