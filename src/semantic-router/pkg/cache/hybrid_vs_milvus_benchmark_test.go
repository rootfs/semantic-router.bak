...
func BenchmarkHybridVsMilvus(b *testing.B) {
	...
	b.Logf("This benchmark validates the claims from the hybrid HNSW storage architecture paper")
	b.Logf("Expected results:")
	b.Logf("  - Hybrid: 6x faster cache hits (42ms → 7ms)")
	b.Logf("  - Hybrid: 60x faster vector search (30ms → 0.5ms)")
	b.Logf("  - Hybrid: 70%% fewer database calls")
	b.Logf("  - Hybrid: 96%% less memory per entry")
	b.Logf("")
	...
	for _, cacheSize := range cacheSizes {
		b.Run(fmt.Sprintf("CacheSize_%d", cacheSize), func(b *testing.B) {
			...
			// Additional search queries (80% should hit, 20% should miss)
			searchQueries := make([]string, 100)
			for i := 0; i < 80; i++ {
				// Hit: use existing queries
				searchQueries[i] = testQueries[i%cacheSize]
			}
			for i := 80; i < 100; i++ {
				// Miss: use new queries
				searchQueries[i] = generateQuery(MediumContent, cacheSize+i)
			}

			// ============================================================
			// 1. Benchmark Pure Milvus Cache
			// ============================================================
			...
			time.Sleep(2 * time.Second)
				...
				b.Logf("Running search benchmark...")
