...
fn test_classification_output_scenarios() {
    ...
    let categories = vec!["biology", "chemistry", "physics", "math", "computer science", "other"];
    for (i, (cat, prob)) in categories.iter().zip(bio_result.probabilities.iter()).enumerate() {
    ...
    assert_eq!(bio_result.confidence, bio_result.probabilities[bio_result.class as usize]);
    ...
    println!("   Category: {} (class {})", math_result.category, math_result.class);
    println!("   Confidence: {:.1}%", math_result.confidence * 100.0);
    println!("   Top 3 categories:");
    let mut indexed: Vec<(usize, f32)> = math_result.probabilities
        .iter()
        .enumerate()
        .map(|(i, &p)| (i, p))
        .collect();
    ...
    println!("   Category: {} (class {})", ambiguous_result.category, ambiguous_result.class);
    println!("   Confidence: {:.1}% ⚠️ Low confidence - ambiguous", ambiguous_result.confidence * 100.0);
    println!("   Distribution shows uncertainty:");
    let categories2 = vec!["biology", "chemistry", "physics", "math", "computer science", "engineering"];
    for (i, (cat, prob)) in categories2.iter().zip(ambiguous_result.probabilities.iter()).enumerate() {
        let marker = if i == ambiguous_result.class as usize { "→" } else { " " };
        println!("     {} [{:>2}] {:20} {:>6.2}%", marker, i, cat, prob * 100.0);
    }
    ...
    println!("   Category: {} (class {})", multi_result.category, multi_result.class);
    ...
    for (q, result) in questions.iter().zip(batch_results.iter()) {
        println!("   \"{}\"\n     → {} ({:.1}%)", q, result.category, result.confidence * 100.0);
    }
    ...
    println!("   Confidence range: {:.1}% - {:.1}%", 
        batch_results.iter().map(|r| r.confidence).fold(f32::INFINITY, f32::min) * 100.0,
        batch_results.iter().map(|r| r.confidence).fold(f32::NEG_INFINITY, f32::max) * 100.0
    );
