...
class Scenario(abc.ABC):
    ...
    def display_iteration(
        self, iteration: int, results: list[dict],
        diagnoses: list[dict], fix: Any,
    ) -> None:
...
class TuningLoop:
    ...
    Works with any Scenario implementation.  The loop:
      ...
      2. Diagnoses failures using the causal engine
    ...
    def run(self) -> dict:
        ...
        for iteration in range(self.max_iterations):
            ...
            if correct == total:
                ...
                iterations.append(self._iteration_record(
                    iteration, results, [], None))
                trajectory.append({
                    "iteration": iteration, "accuracy": correct,
                    "total": total, "pct": pct,
            ...
            iterations.append(self._iteration_record(
                iteration, results, diagnoses, fix))
            trajectory.append({
                "iteration": iteration, "accuracy": correct,
                "total": total, "pct": pct,
                "severity_weighted_loss": weighted_loss,
        ...
        print(f"{'='*60}")
        final_results = self.router.run_probes(probes, adapter)
        self._print_summary("final", final_results)

        final_correct = sum(1 for r in final_results if r["correct"])
        final_weighted_loss = sum(
            self.cs.severity(r) for r in final_results if not r["correct"])

        output = {
            "scenario": self.cs.name,
            "method": "analytical_causal_tracing",
            ...
            "final_verification": {
                ...
                "pct": round(100 * final_correct / len(final_results), 1)
                if final_results else 0,
                "severity_weighted_loss": final_weighted_loss,
            },
