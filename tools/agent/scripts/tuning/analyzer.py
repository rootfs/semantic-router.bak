...
class OfflineAnalyzer:
    ...
    def find_optimal_threshold(
        ...
        for tau in candidates:
            ...
            correct = (sum(1 for q in kept if correct_small_fn(q)) +
                       sum(1 for q in escalated if correct_large_fn(q)))
            uplifts = sum(1 for q in escalated if quadrant_fn(q) == "uplift")
            regressions = sum(
                1 for q in escalated if quadrant_fn(q) == "regression")
            ...
            all_candidates.append({
                ...
                "uplifts": uplifts, "regressions": regressions, "net": net,
                "severity_net": fix.net_improvement,
            })

            if (correct > best_accuracy or
                    (correct == best_accuracy
                     and len(escalated) < best_esc_rate * len(items))):
